...
def _crop_rgb_to_event_fov(rgb_image, K_rect_event, K_rect_rgb):
    ...
    x0, x1, y0, y1 = _event_fov_crop_box(K_rect_event, K_rect_rgb)    
...
def process_rgb_image(rgb_image_path, K_rect_event, K_rect_rgb, image_size=(384, 384)):
    ...
    # Process the RGB image: read, convert to RGB, crop to event FOV, resize.
    # note: stored as raw uint8 CHW, teacher-agnostic. The dataloader does
    # `/ 255.0`; For the teacher in phase-1 (DINOv3), apply
    # ImageNet mean/std in the dataloader / teacher wrapper -- NOT here.
    img = cv2.imread(str(rgb_image_path))
    if img is None: 
        raise FileNotFoundError(f"RGB image not found at {rgb_image_path}")
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

    img_cropped = _crop_rgb_to_event_fov(img, K_rect_event, K_rect_rgb)
...
def process_sequence(seq, rgb_dir, events_dir, calibrations_dir, pairs_dir, preprocessed_rgb_dir,
    ...
    with h5py.File(seq_events_file, 'r') as h5f:
        ...
        for i in range(1, len(timestamps_us), frame_stride):
            ...
            x_r, y_r, p, t = rectify_event_coords(sliced_events, rectify_map)
...
def build_pairs(cfg: DictConfig):
    ...
    for seq in sequences:
        # processing sequentially for each sequence (total: 41 sequences)
        try:
