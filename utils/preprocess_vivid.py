...
def process_sequence(seq, bag_path, pairs_dir, out_root, cfg, overlay_dir=None):
    ...
    def emit(t_us, img_buf):
        ...
        hot = compute_hot_pixel_mask(x_r, y_r, sensor_hw=sensor_hw,
                                     threshold=dcfg.hot_pixel_threshold)
    ...
    with AnyReader([bag_path]) as reader:
        ...
        next_target = None
        # Progress in BAG-SECONDS, not frames: the cost of this loop is bz2
        # chunk decompression, which is near-uniform per second of recording,
        # so this gives a usable ETA. Emitted frames are gated by
        # min_events_per_px / exposure and arrive lumpily, which would not.
        bar = tqdm(total=max(int(dur_s), 1), unit='s', desc=f'{seq:<28}',
                   dynamic_ncols=True, smoothing=0.05)
        t0_ns, n_msg = reader.start_time, 0
        for conn, t_ns, raw in reader.messages(connections=conns):
            n_msg += 1
            if n_msg % 200 == 0:   # ~3 bag-s; keeps formatting off the hot path
                bar.n = min(int((t_ns - t0_ns) / 1e9), bar.total)
                bar.set_postfix(kept=state['kept'], skipped=n_skipped, refresh=False)
                bar.refresh()
...
def build_pairs(cfg: DictConfig):
    ...
    total = 0
    with open(out_root / cfg.datasets.pairs_name, 'w') as master_f:
        for bag in bags:
