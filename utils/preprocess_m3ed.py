...
the same output format as preprocess_ddd20.py: <seq>/rgb/ + <seq>/events/
histogram/ .npy files plus a master pairs.txt with the 6-column DSEC line
format (only the histogram column is materialized; the voxel/timesurface
columns are placeholder paths that are never written).
    ...
    with h5py.File(h5_path, 'r') as f:
        ...
        for fi in candidates:
            ...
            hist = process_events(x_r, y_r, p, box, out_size,
                                  dcfg.event_hot_pixels_threshold)
            if hist is None:
